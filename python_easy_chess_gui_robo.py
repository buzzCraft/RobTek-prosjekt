...
import gameObserver #Handel camera and stuff
import socket #For abb connection
...
# (1) Mode: Neutral
menu_def_neutral = [
        ['&Mode', ['Play','Connect to ABB', 'RoboGame']],
...
class EasyChessGui:
    ...
    def play_game(self, window, engine_id_name, board):
        ...
        while not board.is_game_over(claim_draw=True):
            ...
            if is_human_stm:
                ...
                while True:
                    ...
                    if self.robogame:
                        ...
                        #Vent på signal fra robostudio at vi kan ta bilde
                        window.find_element('comment_k').Update("Waiting for user to make a move and push ok")
                        
                        #Klare for å motta trekk
                        mov = self.get_move(window)
                        #Sender trekket tilbake til robot, behandler bildet og får det tilbake
                        mov = self.calculate_move(mov, window)
                        ...
                        fr_row, fr_col, to_row, to_col = mov #pakker ut move til fire variable
                        move_from=(fr_row,fr_col)
                        move_to = (to_row, to_col)
                        # print(fr_row, fr_col,to_row, to_col )
                      ...
                      #  fr_row, fr_col = move_from
                        ...
                        if user_move in board.legal_moves:
                           ...
                           # print(user_move)
                            # if self.connected:
                            #     self.client.send(bytes(str(user_move).upper(), 'utf-8')) #Send move
                            ...
                            #     window.find_element('comment_k').Update("Robot done moving")
                            #     break
                            ...
                            # SEND MOVE OK #
                            ################
                            print("Sending move ok")
                            self.send_msg("r"+mov_type)
                            if (self.get_msg() != "r"):
                                print("Error in robot moving line 2194")
                            else:
                        ...
                        # Else if move is illegal
                            ...
                            # Restore the color of the fr square
                            button_square.Update(button_color=('white', color))
                            window.find_element('comment_k').Update("Wrong move!")
            ...
            # Else if side to move is not human
                ...
                ###SENDER SVART MOVE
                self.send_black(mov_type, str(best_move).upper())
    ...
    def get_move(self, window):
        #Sender til robot at vi er klare for å hente et nytt trekk
        self.send_msg("k") 
        #Robot sender tilbake - Jeg er også klar, ta bilde du
        data = self.get_msg()  #Får beskjed fra robotstudio
        print(data)
        #Sjekker at vi har fått ok bilde
        if data == ("OKBilde"):
            #Oppdaterer GUI
            window.find_element('comment_k').Update("Tar bilde og finner flytt")
            #Tar bilde og finner flytt
            mov = self.go.main_task()  
            movCheck = re.split(';|,',mov)
            print(movCheck)
            #Sjekker at vi får tilbake en liste med 4 elementer
            if len(movCheck) == 5:
                #retunerer flyttet
                return mov

        else:
            #Feil data fra rob gir:
            window.find_element('comment_k').Update("Expected OKBilde, got " + data) 
            print("Expected OKBilde, got " + data)
        return False
    
    def calculate_move(self, mov, window):
        #sender bildeverdier til robot, omregning gjøres der
        print("Debug - Calculate move")
        print("Sending")
        self.send_msg(mov)
        print("Sent, reciving")
        #mottar flyttet fra robot
        mov = self.get_msg()
        print("Recived " + mov)
        mov = re.split(';|,',mov)
        mov = [int(item) for item in mov] #konverterer til INT
        mov = self.celToColRow(mov) #konvertere til celler og rad
        return mov
    
    def celToColRow(self, mov):
        #Row = int(mov[0]/8)
        #Cell = mov[0]%8
        
        f = mov[0]-1
        t = mov[1]-1
        mov = [int(f/8),int(f%8),int(t/8),int(t%8)]
        return mov
    ...
    def get_msg(self):
        """ Recive a string from robotstudio """
        data = self.client.recv(1024)
        data = data.decode('utf-8')
        
        return data
    
    def send_black(self, mov_type, mov):

        f_x = (ord(mov[0])-65) # Tar ASCII verdien av bokstav, - 65 (siden A = 65)
        f_y = 8-int(mov[1])
        t_x = (ord(mov[2])-65) # Tar ASCII verdien av bokstav, - 65 (siden A = 65)
        t_y = 8-int(mov[3])
        mov = "s"+mov_type+";"+str(f_y)+","+str(f_x)+";"+str(t_y)+","+str(t_x)
        self.send_msg(mov)
        print(mov)
        ans=self.get_msg()
        if ans =="OKBilde":
    ...
    def main_loop(self):
        ...
        while True:
            ...
            #Connect to ABB
            if button == 'Connect to ABB':
                win_title = 'Connect to robotstudio / live robot'
                layout = [
                        [sg.T('IP', size=(4,1)), sg.Input(
                                self.IP, key='ip', size=(32,1))],
                        [sg.T('Port', size=(4,1)), sg.Input(
                                self.port, key='port', size=(4,1))],
                        [sg.Button('Connect'), sg.Cancel()]
                ]
                window.Disable()
                w = sg.Window(win_title, layout,
                              icon=ico_path[platform]['pecg'])
                while True:
                    ...
                    if e == 'Connect':
                        ...
                        try:
                            ...
                            data = self.client.recv(1024)
                            window.find_element('comment_k').Update(data.decode('utf-8'))
                            self.connected = True
                        except:
                            window.find_element('comment_k').Update("Couldn't connect. Check server and IP")
            ...
            #Play with robot
            if button == 'RoboGame':
                if self.connected:           #Check to see if connected
                    self.robogame = True 
                    ...
                    while True:
                        ...
                        window.find_element('_gamestatus_').Update('Mode     RoboGame')
                        window.find_element('_movelist_').Update(disabled=False)
                        window.find_element('_movelist_').Update('', disabled=True)
                    ...
                    window.find_element('comment_k').Update("Connect to ABB robot first!")
