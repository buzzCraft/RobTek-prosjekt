...
#
# Karl Skretting, UiS, February 2019, November 2020 (need Python 3.6 ->)
...
try:
	...
	# raise ImportError("%s: Requires qimage2ndarray." % _appFileName)
	print("%s: Import error for qimage2ndarray. Continue without it." % _appFileName)
	q2n_ok = False
...
def np2qimage(B):
	...
	if q2n_ok and ((len(B.shape) == 2) or ((len(B.shape) == 3) and (B.shape[2] == 1))):
		...
		# print('np2qimage: write image B (numpy array) to file %s' % tempFile)
		cv2.imwrite(tempFile, B)
		# print('np2qimage: and read it from file %s into qImage' % tempFile)
...
def testAll():
	...
	if isinstance(b, np.ndarray):
		print("  smoothFilter(5) return b as np.ndarray of %s, shape %s." % (b.dtype.name, str(b.shape))) 
		print(("  returned values are: "+len(b)*"%.4f, ") % tuple(b))
	#
	ok = True
	...
	if ok:
		...
		if isinstance(qImage, QImage) and (not qImage.isNull()):
			print( '  the returned qImage has format %i, height %i and width %i.' % 
					(qImage.format(), qImage.height(), qImage.width()) )
		else:
	...
	if ok:
		if (isinstance(B, np.ndarray) and (len(B.shape)==3) and (B.shape[2]==3) and
			...
			if ((0 <= x) and (x < B.shape[0]) and (x < C.shape[0]) and
				...
				print("  B[%i,%i] = [%i,%i,%i],  C[%i,%i] = [%i,%i,%i,%i]" % 
					(xy + tuple(B[x,y,:]) + xy + tuple(C[x,y,:])) )
			#
		#
	#end if
	#
